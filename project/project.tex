...
Because our number of features were relatively modest, we wanted to minimize the number of touches and taps you'd have to do to get to a wanted feature. These features are sketched in figure 1.
...
We found out, that people didn’t understand the first icon (top-left icon on figure 1), so we found another icon based on suggestions from evaluation. \\
...
We conducted it with an indirect observation technique, where we recorded what the evaluator said. See the transcript of the record on page \pageref{transcript}. 
...
We did a heuristic evaluation on the hi-fi prototype. A fully description of the evaluation and it's fully result, can be found on page \pageref{heuristic}. \\
An summery of the result can be found in section \ref{result}.
\section{Result}\label{result}
\subsection*{Quick \& dirty}


\subsection*{Think aloud}
From the recorded evaluation (see transcript on page \pageref{transcript}), we got the following result on the question from the latter section:
\begin{itemize}
\item \textbf{Can the user track and find a friend?}\\
	Yes, there was no problem with this. 
\item \textbf{Can the user turn on/off location broadcasts?}\\
	No. The evaluator thought, that the icon for the button to toggle the broadcast, was an icon for sound (probably a speaker icon). This is a clear indication, that we need another icon, which is better to show its purpose. \\
	Our evaluator suggested that we could use the same icon as the Facebook Messenger app uses, to turn geolocation on and off. We think it's a good idea to try. 
\item \textbf{Can the user track a friend from a notification?}\\
	Yes, there was no problem with this.
\item \textbf{Can the user make a SMS/call/nudge from the app?}\\
	Yes, there was no problem with this.
\item \textbf{Can the user call a friend from a notification?}\\
	No, the evaluator had some problems with this. He couldn't expand the notification so the call-action button became visible. 
As the evaluator isn't used to Android (he is an iPhone user) we think this is the reason. When we showed him how to do, he agreed that a regular Android user wouldn't have this problem. 
\item \textbf{Can the user asses the distance from a given user?}\\
	Yes, there was no problem with this. 
\end{itemize}

\subsection*{Heuristic}
We did a heuristic evaluation (see page \pageref{heuristic}) and the following is a very short and compact summery of the findings from the evaluation. To see the fully result, see page \pageref{heuristic}. \\

Our app is generally very simple and easy to use for the user. Although we need more of the following:
\begin{itemize}
\item More Toasts to give the user feedback
\item Other icons for certain buttons (already mentioned i Think aloud evaluation)
\item The app need to comply more with Androids material design patterns. 
\item Help to recover from error is needed (thus not critical) and help with the use of the app in general.  
\end{itemize}
\section{Discussion}
